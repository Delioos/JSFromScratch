...
//5
const array = ['banana','mango','orange','banana','apple','grapes']
const set = new Set(array)
console.log(set)

//6
const ctr = new Map(countries.map(country => [country.name, country.name.length]))
console.log(ctr)
...
a.forEach(n => b.has(n) ? intArr.push(n) : intArr)
const intersec = new Set(intArr)
console.log(intersec)

// ex 3
// 1 How many languages are there in the countries object file?
let allLang = countries.map(country => country.languages)
let lang = new Set(allLang)
console.log(lang.size) // 250

// 2 *** Use the countries data to find the 10 most spoken languages:
let langCount = new Map()
countries.forEach(country => {
    country.languages.forEach(language => {
        if (langCount.has(language)) {
            langCount.set(language, langCount.get(language) + 1)
        } else {
            langCount.set(language, 1)
        }
    }
    )
}
...
let langCountArr = Array.from(langCount)
langCountArr.sort((a, b) => b[1] - a[1])
console.log(langCountArr.slice(0, 10))
// gg wp github copilot on a rien pu faire 